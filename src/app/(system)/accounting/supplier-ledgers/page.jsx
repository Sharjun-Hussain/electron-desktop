...
export default function SupplierLedgersPage() {
  ...
  const fetchLedger = async (supplierId) => {
    ...
    try {
      setLoading(true);
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/suppliers/${supplierId}/ledger`,
        {
  ...
  const chartData = useMemo(() => {
    ...
    return sorted.map((item) => {
      ...
      if (item.type === "credit")
        runningBalance += amount; // Purchasing increases payable
  ...
  const stats = useMemo(() => {
    const settlementRatio =
      totalPurchased > 0 ? (totalSettled / totalPurchased) * 100 : 0;
    return {
  ...
  const exportData = useMemo(() => {
    ...
    return ledger.map((item) => ({
      ...
      "Debit (Paid)": item.type === "debit" ? parseFloat(item.amount) : 0,
      "Credit (Purchased)": item.type === "credit" ? parseFloat(item.amount) : 0,
    ...
    <div className="p-4 md:p-6 space-y-6 min-h-screen bg-transparent">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-foreground">Supplier Ledger</h1>
            <p className="text-sm text-muted-foreground mt-0.5">
              Analyze payables, settlement history and liability turnover
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <DataActions
            data={exportData}
            fileName={`Supplier_Ledger_${suppliers.find((s) => String(s.id) === selectedSupplier)?.name?.replace(/\s+/g, "_") || "Export"}`}
          />
          <Button
            className="h-9 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-4 rounded-md shadow-sm"
                ...
                <PopoverContent className="w-[400px] p-0 shadow-md" align="start">
                  <Command>
                    ...
                    <CommandList className="max-h-[250px]">
                      <CommandEmpty className="py-4 text-center text-sm text-muted-foreground">No supplier found.</CommandEmpty>
                      <CommandGroup>
                        {suppliers.map((s) => (
                          <CommandItem
                            key={s.id}
                            value={s.name}
                            onSelect={() => { setSelectedSupplier(String(s.id)); fetchLedger(String(s.id)); setOpen(false); }}
                            className="py-2 px-3 flex items-center justify-between cursor-pointer text-sm"
                          >
                            <div className="flex flex-col">
                              <span className="font-semibold text-foreground">{s.name}</span>
                              <span className="text-xs text-muted-foreground">{s.phone || "No phone recorded"}</span>
                            </div>
                            <Check className={cn("h-4 w-4 text-emerald-500", selectedSupplier === String(s.id) ? "opacity-100" : "opacity-0")} />
                          </CommandItem>
                        ))}
                      </CommandGroup>
                    </CommandList>
            ...
            <div className="flex flex-col gap-6">
                ...
                <CardContent className="p-5 space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="p-4 rounded-md bg-muted/30 border border-border">
                      ...
                      <p className="text-lg font-bold">
                        LKR {totalPurchased.toLocaleString()}
                      </p>
                    </div>
                    <Button
              ...
              <Card className="border-border shadow-sm rounded-xl overflow-hidden bg-card/40 backdrop-blur-xl p-5 space-y-3">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-semibold">Payable Audit</p>
                    <p className="text-xs text-muted-foreground font-medium">
                      REF ID: SUP-{selectedSupplier}
                    </p>
                  </div>
                </div>
                <div className="space-y-3 pt-3 border-t border-border/50">
                        ...
                        <UITooltip>
                          ...
                          <TooltipContent className="max-w-[280px] p-3 shadow-2xl border border-border/80 bg-popover text-popover-foreground animate-in fade-in zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out data-[state=closed]:zoom-out-95">
                            <p className="text-[12px] leading-relaxed font-medium">
                              Measures the frequency and reliability of your payments to this supplier.
                              A high velocity indicates healthy vendor relationships and strong liability management.
                            </p>
                          </TooltipContent>
                        </UITooltip>
                      </TooltipProvider>
                  ...
                  <div className="w-full h-1.5 bg-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${stats.settlementRatio}%` }}
                    />
                  </div>
                </div>
              </Card>
              ...
              <Table>
                <TableHeader className="bg-muted/30">
                  <TableRow className="hover:bg-transparent border-border/60">
                    <TableHead className="text-[11px] font-bold uppercase text-muted-foreground/70 py-3 px-6">
                      Transaction Date
                    </TableHead>
                    <TableHead className="text-[11px] font-bold uppercase text-muted-foreground/70 py-3 px-4">
                      Reference
                    </TableHead>
                    <TableHead className="text-[11px] font-bold uppercase text-muted-foreground/70 py-3 px-4 text-right">
                      Debit (Paid)
                    </TableHead>
                    <TableHead className="text-[11px] font-bold uppercase text-muted-foreground/70 py-3 px-4 text-right">
                      Credit (Liability)
                    </TableHead>
                    <TableHead className="text-[11px] font-bold uppercase text-muted-foreground/70 py-3 px-6 text-right">
                      Balance
                    </TableHead>
                  </TableRow>
                </TableHeader>
                  ...
                  {loading ? (
                    ...
                    ledger.map((item, idx) => (
                      ...
                      >
                        ...
                        <TableCell className="py-3 px-4 text-right font-medium text-sm text-rose-600 tabular-nums">
                          {item.type === "credit"
                            ? `LKR ${parseFloat(item.amount).toLocaleString()}`
                            : "—"}
                        </TableCell>
      ...
      <Sheet open={paymentOpen} onOpenChange={setPaymentOpen}>
        <SheetContent className="sm:max-w-md w-full p-0 border-l border-border bg-background">
          <div className="h-full flex flex-col relative overflow-hidden">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="relative">
                          ...
                          <Input
                            ...
                            className="h-10 pl-12 text-base font-bold shadow-sm focus:ring-emerald-500/20"
                            required
                          />
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              setPaymentAmount(Math.abs(balance) / 2)
                            }
                            className="h-8 shadow-sm text-xs"
                          >
                            50% Impact
                      ...
                      <div className="p-4 rounded-md bg-muted border border-border flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-lg font-bold text-foreground">
                            LKR{""}
                            {(
                              Math.abs(balance) - paymentAmount
                            ).toLocaleString()}
                          </p>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          {[
                            ...
                            <button
                              ...
                              className={cn(
                                "flex flex-col items-center justify-center gap-2 p-3 rounded-lg border transition-all duration-200 shadow-sm",
                                paymentMethod === m.id
                                  ? "bg-emerald-600 border-emerald-600 text-white ring-2 ring-emerald-500/20"
                                  : "bg-background dark:bg-card/40 border-border text-muted-foreground hover:bg-emerald-50/50 dark:hover:bg-emerald-500/5 hover:border-emerald-500/30",
                              )}
                            >
                              <m.icon className={cn("h-4 w-4", paymentMethod === m.id ? "text-white" : "text-emerald-600")} />
                          ...
                          <div className="grid grid-cols-1 gap-3">
                            <div className="space-y-1.5">
                              ...
                              <Input
                                ...
                                className="h-9 shadow-sm focus:ring-emerald-500/20"
                              />
                            </div>
            ...
            <SheetFooter className="p-6 shrink-0 bg-background border-t border-border flex flex-row justify-end gap-3">
              <Button
                type="button"
                variant="outline"
              ...
              <Button
                ...
                className="w-full sm:w-auto min-w-[140px] bg-emerald-600 hover:bg-emerald-700 text-white font-bold shadow-sm"
              >
                {paymentLoading ? (
                  <>
