...
import {
  ...
  Banknote, Star, MousePointerClick
} from "lucide-react";
...
export function CustomerLedgerSheet({ customer, open, onOpenChange, accessToken }) {
  ...
  const fetchLedger = useCallback(async () => {
    if (!customer?.id || !accessToken) return;
    try {
      setLoading(true);
      ...
      console.error(error);
      toast.error("Failed to fetch ledger archive");
    } finally {
      setLoading(false);
    }
  ...
  }, [open, customer?.id, activeTab, fetchLedger, fetchOrders, fetchPurchasedItems]);
  ...
  return (
    <>
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <SheetTitle className="text-xl font-bold text-foreground flex items-center gap-2">
                    {customer?.name}
                    <Badge variant={customer?.is_active ? "default" : "secondary"} className="text-[10px] font-semibold tracking-wide">
              ...
              <div className="text-right">
                ...
                <div className="flex items-center justify-end gap-2">
                  ...
                  <Badge variant="outline" className={cn("text-xs font-bold border", isReceivable ? "text-amber-600 border-amber-200 bg-amber-50" : "text-emerald-600 border-emerald-200 bg-emerald-50")}>
                    {isReceivable ? "DR" : "CR"}
                  </Badge>
                </div>
              </div>
            ...
            <div className="px-8 border-b border-border bg-muted/10 shrink-0">
              <TabsList className="bg-transparent h-12 w-full justify-start gap-6 p-0 border-none">
                {[
                  ...
                  { id: "orders", label: "Logs", icon: ShoppingBag },
                  { id: "ledger", label: "Ledger", icon: Activity },
                  { id: "items", label: "Assets", icon: Package },
                ].map((tab) => (
                  <TabsTrigger
                    key={tab.id}
                    value={tab.id}
              ...
              <TabsContent value="overview" className="mt-0 space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2 pb-2 border-b border-border">
                    ...
                    <h3 className="text-sm font-bold text-foreground">Account Metrics</h3>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Card className="p-4 shadow-sm">
                      <div className="flex items-center gap-2 mb-2">
                        <Banknote className="h-4 w-4 text-emerald-600/70" />
                        <p className="text-xs font-semibold text-muted-foreground">Lifetime Value</p>
                      </div>
                      <p className="text-xl font-bold tabular-nums text-foreground">{formatCurrency(parseFloat(customer?.totalSpent || 0))}</p>
                    </Card>
                    <Card className="p-4 shadow-sm">
                      <div className="flex items-center gap-2 mb-2">
                        <Star className="h-4 w-4 text-emerald-600/70" />
                        <p className="text-xs font-semibold text-muted-foreground">Loyalty Points</p>
                      </div>
                      <p className="text-xl font-bold tabular-nums text-foreground">{customer?.loyaltyPoints || 0} pts</p>
                    </Card>
                    <Card className="p-4 shadow-sm">
                      <div className="flex items-center gap-2 mb-2">
                        <MousePointerClick className="h-4 w-4 text-emerald-600/70" />
                        <p className="text-xs font-semibold text-muted-foreground">Total Visits</p>
                      </div>
                      <p className="text-xl font-bold tabular-nums text-foreground">{customer?.visits || 0}</p>
                    </Card>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2 pb-2 border-b border-border">
                    ...
                    <h3 className="text-sm font-bold text-foreground">Financial Actions</h3>
                  </div>
                  ...
                  <Table>
                    <TableHeader className="bg-muted/50">
                      <TableRow>
                        ...
                        <TableHead className="font-semibold text-[11px] uppercase tracking-wider">Invoice</TableHead>
                        ...
                        <TableHead className="text-right font-semibold text-[11px] uppercase tracking-wider">Payable</TableHead>
                        <TableHead className="text-right font-semibold text-[11px] uppercase tracking-wider">Paid</TableHead>
                        <TableHead className="text-right font-semibold text-[11px] uppercase tracking-wider pr-4">Balance</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {loading && ordersData.length === 0 ? (
                        ...
                        <TableRow><TableCell colSpan={7} className="h-24 text-center text-sm text-muted-foreground">No logs found.</TableCell></TableRow>
                      ) : (
                        ...
                        ledgerData.map((t) => (
                          <TableRow key={t.id} className="transition-colors hover:bg-muted/30">
                            ...
                            <TableCell className="text-right">
                              <span className={cn("text-sm font-medium", t.type === "debit" ? "text-amber-600" : "text-emerald-600")}>
      ...
      {/* SETTLEMENT DIALOG - Fixed for small screens */}
        ...
        <DialogContent className="sm:max-w-lg p-0 bg-background border shadow-xl flex flex-col max-h-[90vh] overflow-hidden">
          ...
          <DialogHeader className="px-6 py-5 border-b border-border bg-background shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">Reconcile current balance for {customer?.name}</p>
              </div>
            </div>
          </DialogHeader>
          ...
          <form id="settlement-form" onSubmit={handleSettleSubmit} className="flex-1 flex flex-col min-h-0">
            ...
            <div className="p-6 space-y-6 overflow-y-auto">
              <div className="space-y-2">
                <Label htmlFor="amount" className="font-semibold">Reconciliation Amount</Label>
                <Input
