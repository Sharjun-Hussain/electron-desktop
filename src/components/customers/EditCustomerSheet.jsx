...
import { z } from "zod";
import { Loader2, UserCog, X, MapPin, Mail, Phone, User, CheckCircle2 } from "lucide-react";
...
export function EditCustomerSheet({ customer, open, onOpenChange, onSave }) {
  ...
  const onSubmit = async (data) => {
    if (!session?.accessToken || !customer) return;
      ...
      <SheetContent className="sm:max-w-[540px] p-0 flex flex-col h-full border-l border-border/50">
        <SheetHeader className="px-8 py-6 border-b border-border bg-background shrink-0">
          <div className="flex items-center gap-3 mb-1">
            ...
            <SheetTitle className="text-xl font-bold text-foreground truncate max-w-[320px]">
              {customer?.name || "Edit Customer"}
            </SheetTitle>
          </div>
          <SheetDescription className="text-sm text-muted-foreground">
            Update the core details and contact information for this client.
          </SheetDescription>
        </SheetHeader>
