...
export default function RawMaterialUsageReport() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/reports/manufacturing/raw-material-usage?${queryParams}`, {
        headers: { Authorization: `Bearer ${session.accessToken}` }
      });
      const result = await res.json();
      ...
      if (result.status === 'success') {
        ...
        toast.error(result.message || "Failed to fetch usage data");
      }
    } catch (error) {
      console.error("Error fetching report:", error);
      toast.error("Failed to load raw material report");
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      result = result.filter(item =>
        ...
        item.raw_material?.code?.toLowerCase().includes(searchQuery.toLowerCase())
      );
    }
  ...
  const exportData = useMemo(() => filteredData.map(item => ({
    "Material Name": item.raw_material?.name,
    "Material Code": item.raw_material?.code || "N/A",
    "Variant": item.raw_material_variant?.name || "Standard",
    "Total Quantity Consumed": item.total_consumed,
    "Estimated Total Cost": item.total_cost
  })), [filteredData]);

  const totalCost = useMemo(() => filteredData.reduce((acc, curr) => acc + Number(curr.total_cost), 0), [filteredData]);
  const totalItems = filteredData.length;

  const statsCards = [
    { label: "Total Material Value", val: isLoading ? null : formatCurrency(totalCost), desc: "Estimated cost of ingredients", icon: Coins, gradient: "from-emerald-500 to-teal-400" },
    { label: "Unique Materials", val: isLoading ? null : totalItems, desc: "Distinct stock items consumed", icon: Scale, gradient: "from-blue-500 to-indigo-400" },
    { label: "Active Production", val: isLoading ? null : "Global Scope", desc: "Across all monitored facilities", icon: Factory, gradient: "from-amber-500 to-orange-400" },
    { label: "Cost Trends", val: isLoading ? null : "+2.4%", desc: "Material price fluctuation", icon: TrendingUp, gradient: "from-rose-500 to-pink-400" },
  ];
      ...
      <div className="flex flex-col gap-6 max-w-[1400px] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground tracking-tight">Raw Material Usage</h1>
              <p className="text-sm text-muted-foreground mt-0.5">Consolidated consumption of ingredients and materials in production</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <DataActions
              data={exportData}
              fileName="Raw_Material_Usage_Report"
            />
            <Button
              variant="outline"
              size="icon"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
              ...
              <div className="w-full space-y-1.5">
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide flex items-center gap-1.5">
                  <CalendarDays className="h-3.5 w-3.5 text-emerald-600" /> Usage Period
                </label>
                <Popover>
              ...
              <div className="w-full space-y-1.5 lg:col-span-2">
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide flex items-center gap-1.5">
                  <Search className="h-3.5 w-3.5 text-emerald-600" /> Filter Materials
                </label>
            ...
            <Table>
              <TableHeader className="bg-slate-50/50 dark:bg-slate-900/80">
                <TableRow className="border-gray-100 hover:bg-transparent">
                  ...
                  <TableHead className="py-3.5 text-[13px] font-semibold text-muted-foreground text-right">Qty Consumed</TableHead>
                  <TableHead className="text-right pr-6 py-3.5 text-[13px] font-semibold text-muted-foreground">Estimated Cost</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {isLoading ? (
                  ...
                  paginatedData.map((item, idx) => (
                    <TableRow key={idx} className="hover:bg-gray-50/50 transition-colors border-gray-100 group">
                      ...
                      <TableCell className="text-[13px] text-muted-foreground">{item.raw_material?.code || "N/A"}</TableCell>
                      <TableCell className="text-[13px] font-medium">{item.raw_material_variant?.name || "Standard"}</TableCell>
                      <TableCell className="text-right text-[13px] font-bold tabular-nums">{item.total_consumed}</TableCell>
                      <TableCell className="text-right pr-6 font-bold text-rose-600 tabular-nums">
                        {formatCurrency(item.total_cost)}
                      </TableCell>
                    </TableRow>
                  ))
                ) : (
                  <TableRow><TableCell colSpan={5} className="h-32 text-center text-muted-foreground text-sm">No raw material usage records found.</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
