...
export default function DailySalesSummaryPage() {
  ...
  const PaymentBar = () => {
    ...
    const methods = Object.keys(breakdown).sort((a, b) => breakdown[b] - breakdown[a]);
    ...
    return (
      <div className="w-full flex flex-col justify-center gap-3">
          ...
          {methods.map((method) => (
            breakdown[method] > 0 && (
              <div 
                ...
                className={cn(getColor(method), "h-full transition-all duration-700")} 
              />
            )
          ))}
        </div>
        <div className="flex flex-wrap gap-x-3 gap-y-1 text-[10px] font-semibold text-muted-foreground max-h-12 overflow-y-auto">
          {methods.map((method) => (
            breakdown[method] > 0 && (
              <div key={method} className="flex items-center gap-1.5 shrink-0">
                <div className={cn("size-2 rounded-full", getColor(method))} />
                <span className="capitalize">{method}</span> {breakdown[method]}%
              </div>
            )
          ))}
        </div>
  ...
  const statsCards = [
    ...
    },
    {
      ...
      val: isLoading ? null : formatCurrency(Math.abs(stats.totalDiscounts || 0)),
                 ...
                 <Popover>
                    ...
                    <PopoverContent className="w-80 p-6 rounded-md border-gray-200 shadow-xl" align="center">
                      <div className="space-y-6">
                        ...
                        <Button variant="ghost" size="sm" className="w-full h-8 rounded-md text-xs font-medium text-rose-500 hover:bg-rose-50" onClick={() => {setPaymentFilter("all"); setAmountRange([0,5000])}}>Reset Filters</Button>
                      </div>
                    </PopoverContent>
                  </Popover>
              </div>
            ...
            <Table>
              <TableHeader className="bg-slate-50/50 dark:bg-slate-900/80">
                <TableRow className="border-gray-100 hover:bg-transparent">
                  ...
                  <TableHead className="py-3.5 text-[13px] font-semibold text-muted-foreground text-right">Net Revenue</TableHead>
                  <TableHead className="text-center py-3.5 text-[13px] font-semibold text-muted-foreground">Settlement</TableHead>
                  <TableHead className="text-right pr-6 py-3.5 text-[13px] font-semibold text-muted-foreground">Auth Personnel</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {isLoading ? (
                    ...
                    return (
                      <TableRow key={item.id} className={cn(
                        ...
                        <TableCell className="pl-6 py-3.5 relative">
                           {isCredit && <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-amber-500 shadow-[2px_0_10px_rgba(245,158,11,0.2)]" />}
                           <p className="text-[13px] font-semibold text-muted-foreground tabular-nums">{formatDateTime(item.date)}</p>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-center">
                            <div className="flex justify-center flex-wrap gap-1 max-w-[120px] mx-auto">
                              {item.payments && item.payments.length > 0 ? (
                                item.payments.map((pmt, idx) => (
                                  <Badge key={idx} variant="outline" className={cn(
                                      ...
                                      pmt.payment_method?.toLowerCase() === 'card' ? "bg-blue-50 text-blue-600" :
                                      pmt.payment_method?.toLowerCase() === 'cash' ? "bg-emerald-50 text-emerald-600" :
                                      pmt.payment_method?.toLowerCase() === 'cheque' ? "bg-purple-50 text-purple-600" :
                                ...
                                <Badge variant="outline" className={cn(
                                    "text-[9px] font-bold h-5 px-2 shadow-none border-none rounded-md transition-all",
                                    item.type?.toLowerCase() === 'card' ? "bg-blue-50 text-blue-600" :
                                    item.type?.toLowerCase() === 'cash' ? "bg-emerald-50 text-emerald-600" :
                                    item.type?.toLowerCase() === 'cheque' ? "bg-purple-50 text-purple-600" :
                                    "bg-gray-100 text-muted-foreground"
                                )}>
                                    {item.type}
                                </Badge>
                              ...
                              {isCredit && (
                                <Badge variant="outline" className="text-[9px] font-bold h-5 px-2 bg-amber-50 text-amber-600 border-none rounded-md">
                        ...
                        <TableCell className="text-right pr-6">
                              <p className="text-[13px] font-medium text-foreground">{item.cashier}</p>
                              <p className="text-[11px] font-medium text-muted-foreground italic">via POS Terminal</p>
                        </TableCell>
                      </TableRow>
                    )
                  })
                  ...
                  <TableRow>
                     <TableCell colSpan={6} className="py-24 text-center">
                        <div className="flex flex-col items-center justify-center gap-3">
                           ...
                           <h4 className="font-bold text-foreground uppercase tracking-tight">No transaction records mapped</h4>
                           <p className="text-sm font-medium text-muted-foreground italic">Clear quick search or adjust advanced filters to expand visibility</p>
                        </div>
                     </TableCell>
                  </TableRow>
                )}
