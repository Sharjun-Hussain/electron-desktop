...
import {
  HoldListDialog, SaleListDialog, StockCheckDialog, OutOfStockWarningDialog,
...
export default function RetailPosPage() {
  ...
  const handleAddToCart = useCallback((item, quantity = 1, skipBatchCheck = false, skipStockWarning = false) => {
    ...
    dispatch({ type: "ADD_ITEM", payload: { product: item, quantity: finalQuantity, batchId: item.batchId, price: item.price || (state.isWholesale ? item.wholesalePrice : item.retailPrice) } });
    playBeep("scan");
    ...
    <>
      ...
      <div className="hidden h-screen flex-col bg-muted/30 font-sans md:flex relative">
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden pr-24">
          ...
          <div className="flex-1 flex flex-col h-full overflow-hidden transition-all duration-300 border-r border-border/40 p-4 bg-muted/5">
            <div className="flex-1 overflow-hidden bg-card rounded-2xl border border-border/40 shadow-sm flex flex-col">
              {!isProductListVisible ? (
                ...
                <ProductGrid
                  ...
                  gridCols="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5 gap-2"
                />
              )}
            </div>
          </div>

          {/* Right Action Column: Cart + Checkout */}
          <aside className="w-full md:w-[50%] lg:w-[50%] xl:w-[45%] 2xl:w-[40%] flex flex-col h-full bg-muted/5 gap-4 p-4 pl-0">
            <div className="flex-1 overflow-hidden bg-card rounded-2xl border border-border/40 shadow-sm flex flex-col">
              <CartPanel
                cart={state.cart}
                dispatch={dispatch}
                ...
                isManufacturing={isManufacturing}
                isRestaurant={isRestaurant}
                compactProductCol={true}
                visibleColumns={receiptSettings?.posTableColumns || ["barcode", "name", "quantity", "mrp", "price", "discount", "discount_percent", "total", "batch", "expire"]}
      ...
      <UtilitySidebar
        ...
        isRestaurant={isRestaurant}
        onAction={(key) => {
          if (key === 'salesReturn') { setActiveDialog('salesReturn'); }
          else if (key === 'hold' || key === 'holdSale') {
            handleHoldSale({
              adjustment: 0,
              selectedEmployeeIds: [],
              generalDiscount: 0,
      ...
      <OutOfStockWarningDialog 
        isOpen={activeDialog === 'outOfStock'} 
        onOpenChange={(open) => setActiveDialog(open ? 'outOfStock' : null)} 
      ...
      <SalesReturnModal 
        isOpen={activeDialog === 'salesReturn'}
        onOpenChange={(open) => setActiveDialog(open ? 'salesReturn' : null)}
        flattenedVariants={flattenedVariants}
        allProducts={allProducts}
      />
