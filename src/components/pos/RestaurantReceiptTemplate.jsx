...
export const RestaurantReceiptTemplate = forwardRef(({ sale, settings, business, branch, terminalName }, ref) => {
  if (!sale) return null;
  ...
  const diningTypeFormatted = sale.dining_type ? sale.dining_type.replace("_", " ").toUpperCase() : "DINE IN";

  // Group items to consolidate appended items on the final receipt
  const groupedItems = Object.values((sale.items || []).reduce((acc, item) => {
    const key = `${item.product_id}_${item.product_variant_id || 'none'}_${item.unit_price}`;
    if (!acc[key]) {
      acc[key] = { ...item };
    } else {
      acc[key].quantity = parseFloat(acc[key].quantity) + parseFloat(item.quantity);
      acc[key].discount_amount = parseFloat(acc[key].discount_amount || 0) + parseFloat(item.discount_amount || 0);
      acc[key].tax_amount = parseFloat(acc[key].tax_amount || 0) + parseFloat(item.tax_amount || 0);
      if (acc[key].manual_discount !== undefined && item.manual_discount !== undefined) {
        acc[key].manual_discount = parseFloat(acc[key].manual_discount) + parseFloat(item.manual_discount);
      }
    }
    return acc;
  }, {}));
  ...
  return (
    <div ref={ref} className="bg-white">
        ...
        {groupedItems.map((item, idx) => {
          ...
          return (
            <tbody key={idx} className="border-b border-dashed border-black last:border-b-0">
              ...
              {item.cooking_notes && (
                <tr>
                  <td colSpan={4} className="pb-1 pl-4 text-[10px] italic font-semibold border-l-2 border-black ml-1">
                    * {item.cooking_notes}
                  </td>
                </tr>
              )}
      ...
      {/* KITCHEN RECEIPT (conditionally rendered) */}
      {(sale.isKOT || (sale.kot_status !== null && sale.sendToKitchen !== false)) && (
        <>
          {!sale.isKOT && <div className="print-page-break" style={{ pageBreakBefore: 'always' }} />}
          <div
            className={cn(
              "bg-white text-black p-4 print:py-0 print:px-3 mx-auto transition-all uppercase",
              paperWidthClass,
            ...
            style={{ fontFamily: fontConfig.fontFamilyStyle, marginTop: '20px' }}
          >
            ...
            <div className="border-b border-dashed border-black pb-2 mb-2 space-y-0.5 font-bold">
              ...
              <div className="flex justify-between">
                <span>TIME:</span>
                <span>{sale.created_at ? format(new Date(sale.created_at), "HH:mm dd/MM") : format(new Date(), "HH:mm dd/MM")}</span>
              </div>
              <div className="flex justify-between">
                <span>SERVER:</span>
                <span>{sale.sellers && sale.sellers.length > 0 ? sale.sellers[0].name : "STAFF"}</span>
              </div>
              <div className="flex justify-between">
                <span>TYPE:</span>
            ...
            <table className="w-full my-4 border-collapse font-bold text-lg leading-tight">
              ...
              {sale.items?.map((item, idx) => {
                ...
                return (
                  <tbody key={idx} className="border-b border-dashed border-black last:border-b-0">
                    <tr>
                      <td className="text-left py-2 pr-2 align-top whitespace-nowrap">{qty.toLocaleString()} x</td>
...
RestaurantReceiptTemplate.displayName = "RestaurantReceiptTemplate";

export default RestaurantReceiptTemplate;
